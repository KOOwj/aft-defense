	...
	<%
		...
		try {
			...
			if (search == null) {
				...
				result = stmt
						.executeQuery("select count(*) from " + cat_db + " where no like '%" + search + "%';");
			}
			while (result.next()) {
			...
			if (search == null) {
				...
				result = stmt.executeQuery(
						"select no from " + cat_db + " where no like '%" + search + "%' order by no asc;");
			}
			...
			if (search == null) {
				result = stmt.executeQuery("select name, hp, depend, no, image_path from " + cat_db + " where step2 >=" + start
						+ " and step2 <=" + end + " order by step2 asc;");
			} else {
				result = stmt.executeQuery("select name, hp, depend, no, image_path from " + cat_db + " where no like '%"
			...
			<div class="searchline">
				<div class="search">
					...
					<form style="display: inline" name="seachform" method="post"
						action="guide_monster.jsp?category=<%=category%>">
		...
		<div class="noticeline">
			<div class="notice">
				...
				<tr>
					...
					<tr>
						<td id="image">
							<img src="<%=result.getString(5) %>" width="126" height="126"/>
						</td>
						<td id="name">
							<%=result.getString(1)%>
						</td>
					...
					<tr id="list">
						<td colspan="5" id="list">
							<%
								if (pg > BLOCK) {
									if (search == null) {
							%> <a href="guide_monster.jsp?category=<%=category%>&pg=1">◀◀</a> <a
							href="guide_monster.jsp?category=<%=category%>&pg=<%=startPage - 1%>">◀</a>
							<%
								} else {
							%> <a href="guide_monster.jsp?category=<%=category%>&pg=1&search=<%=search%>">◀◀</a>
							<a
							href="guide_monster.jsp?category=<%=category%>&pg=<%=startPage - 1%>&search=<%=search%>">◀</a>
							<%
								}
								}
 			...
 			if (search == null) {
 %> <a href="guide_monster.jsp?category=<%=category%>&pg=<%=i%>"><%=i%></a> <%
 	} else {
 %> <a
							href="guide_monster.jsp?category=<%=category%>&pg=<%=i%>&search=<%=search%>"><%=i%></a>
							<%
								}
									}
								}
 	...
 	if (endPage < allPage) {
 		if (search == null) {
 %> <a href="guide_monster.jsp?category=<%=category%>&pg=<%=endPage + 1%>">▶</a>
							<a href="guide_monster.jsp?category=<%=category%>&pg=<%=allPage%>">▶▶</a>
							<%
								} else {
							%> <a
							href="guide_monster.jsp?category=<%=category%>&pg=<%=endPage + 1%>&search=<%=search%>">▶</a>
							<a
							href="guide_monster.jsp?category=<%=category%>&pg=<%=allPage%>&search=<%=search%>">▶▶</a>
							<%
								}
								}
							%>
