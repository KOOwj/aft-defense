	...
	<%
		...
		try {
			...
			ResultSet result = stmt
					.executeQuery("select title, contents from " + cat_db + " where no=" + idx + ";");
			...
			<div class="notice">
				...
				%>
					<form name="modifyform" method="post"
						action="modify_form_ok.jsp?category=<%=category%>&pg=<%=pg%>&idx=<%=idx%>">
						<%
					} else{ 
					%>
						<form name="modifyform" method="post"
							action="modify_form_ok.jsp?category=<%=category%>&pg=<%=pg%>&idx=<%=idx%>&search=<%=search%>">
							<%
					}
					%>
							<tr>
