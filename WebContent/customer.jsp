...
<body>
	<%
		...
		if (category.equals("1vs1")) {
			cat_kor = "1:1문의";
			cat_db = "customer_1vs1";
	%><link rel="stylesheet" type="text/css" href="customer_1vs1.css">
	<%
		}else {
			cat_kor = "버그신고";
			cat_db = "customer_bug";
	%><link rel="stylesheet" type="text/css" href="customer_bug.css">
	<%
		}
		String search = null;
			...
			<div class="searchline">
				<div class="search">
					...
					<form style="display: inline" name="searchform" method="post"
						action="customer.jsp?category=<%=category%>">
			...
			<div class="notice">
				...
				<table>
					...
					<tr>
						...
						%>
						<%
						if(id==null || !(id.equals(result.getString(2)))) {
							if (search == null){
						%>
						<a href="#" onclick="alert('비밀글입니다.')">
						<%
						} else{
						%>
						<a href="#" onclick="alert('비밀글입니다.')">
...
<%}
 ...
 %></td>
					...
					%><tr id="write">
						<td id="list" colspan="2">
							<%
								if (pg > BLOCK) {
									if (search == null) {
							%> <a href="customer.jsp?category=<%=category%>&pg=1">◀◀</a> <a
							href="customer.jsp?category=<%=category%>&pg=<%=startPage - 1%>">◀</a>
							<%
								} else {
							%> <a href="customer.jsp?category=<%=category%>&pg=1&search=<%=search%>">◀◀</a>
							<a
							href="customer.jsp?category=<%=category%>&pg=<%=startPage - 1%>&search=<%=search%>">◀</a>
							<%
								}
								}
 			...
 			if (search == null) {
 %> <a href="customer.jsp?category=<%=category%>&pg=<%=i%>"><%=i%></a> <%
 	} else {
 %> <a
							href="customer.jsp?category=<%=category%>&pg=<%=i%>&search=<%=search%>"><%=i%></a>
							<%
								}
									}
								}
 	...
 	if (endPage < allPage) {
 		if (search == null) {
 %> <a href="customer.jsp?category=<%=category%>&pg=<%=endPage + 1%>">▶</a>
							<a href="customer.jsp?category=<%=category %>pg=<%=allPage%>">▶▶</a> <%
 	} else {
 %> <a
							href="customer.jsp?category=<%=category%>&pg=<%=endPage + 1%>&search=<%=search%>">▶</a>
							<a
							href="customer.jsp?category=<%=category%>&pg=<%=allPage%>&search=<%=search%>">▶▶</a>
							<%
								}
								}
							%>
