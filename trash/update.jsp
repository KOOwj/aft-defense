...
<body>
	<%
		...
		try {
			...
			if (search == null) {
				result = stmt.executeQuery("select count(*) from update_title;");
			} else {
				result = stmt
						.executeQuery("select count(*) from update_title where title like '%" + search + "%';");
			}
			while (result.next()) {
			...
			int sort = 1;
			if (search == null) {
				result = stmt.executeQuery("select no from update_title order by no desc;");
			} else {
				result = stmt.executeQuery(
						"select no from update_title where title like '%" + search + "%' order by no desc;");
			}
			stmt2.executeUpdate("update update_title set step2 = 0;");
			while (result.next()) {
				int stepNum = result.getInt(1);
				stmt2.executeUpdate("update update_title set step2 = " + sort + " where no = " + stepNum + ";");
				sort++;
			}
			result.close();
			stmt2.close();
			...
			if (search == null) {
				result = stmt.executeQuery("select title, date, no from update_title where step2 >=" + start
						+ " and step2 <=" + end + " order by step2 asc;");
			} else {
				result = stmt.executeQuery("select title, date, no from update_title where title like '%" + search
	...
	<div class="all">
		...
		<div class="searchloginline">
			<div class="searchline">
				<div class="search">
					<div>제목 검색</div>
					<form style="display: inline" name="searchform" method="get"
						action="update.jsp?search=<%=search%>">
			...
			<div class="loginline">
				...
				%>
				<button id="login1"><%=name %>님</button>
				<%
					}
				%>
			</div>
		...
		<div class="noticeline">
			<div class="notice">
				<table>
					...
					%>
					<tr>
						<td id="title"><a href="view.jsp?category=<%=category%>&pg=<%=pg%>&idx=<%=idx%>"><%=result.getString(1)%></a>
						<%
		...
		<%
			if(pg>BLOCK) {
				if(search == null){
		%>
			<a href="update.jsp?pg=1">◀◀</a>
			<a href="update.jsp?pg=<%=startPage-1%>">◀</a>
		<%
				} else{
		%>
			<a href="update.jsp?pg=1&search=<%=search%>">◀◀</a>
			<a href="update.jsp?pg=<%=startPage-1%>&search=<%=search%>">◀</a>
		<%
				}
			}
		...
		<%
					...
					if(search == null){
		%>			
					<a href="update.jsp?pg=<%=i %>"><%=i %></a>
		<%
					} else{
		%>				
					<a href="update.jsp?pg=<%=i %>&search=<%=search%>"><%=i %></a>
		<%			
					}
				}
			}
		...
		<%
			if(endPage<allPage){
				if(search == null){
		%>
			<a href="update.jsp?pg=<%=endPage+1%>">▶</a>
			<a href="update.jsp?pg=<%=allPage%>">▶▶</a>
		<%		
				} else{
		%>			
			<a href="update.jsp?pg=<%=endPage+1%>&search=<%=search%>">▶</a>
			<a href="update.jsp?pg=<%=allPage%>&search=<%=search%>">▶▶</a>
